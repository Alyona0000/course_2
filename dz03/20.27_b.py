    ...
    def deal(self, num_players, num_cards):
        ...
        for deck_index, deck in enumerate(self.all_decks):
            ...
            for player_index in range(num_players):
                ...
                #dealt[player_index].append(player_cards)
                players_hands.append(player_cards)
            ...
            #додамо прикуп кожному гравцю 
            #print(players_hands)
            players_hands[0]=exchange_talon(players_hands[0],deck)
            players_hands[1]=exchange_talon(players_hands[1],deck)
            players_hands[2]=exchange_talon(players_hands[2],deck)
            #print(deck)
            dealt[0].append(players_hands[0])
            dealt[1].append(players_hands[1])
            dealt[2].append(players_hands[2])
...
def exchange_talon(hand, talon):
    h2 = np.concatenate([hand, talon])
    hand = np.sort(h2)[::-1]

    suit_size = np.array([
        hand[hand // 100 == 1].size,
        hand[hand // 100 == 2].size,
        hand[hand // 100 == 3].size,
        hand[hand // 100 == 4].size,
    ])

    suit_rank = np.array([
        (hand[hand // 100 == 1] % 100).sum(),
        (hand[hand // 100 == 2] % 100).sum(),
        (hand[hand // 100 == 3] % 100).sum(),
        (hand[hand // 100 == 4] % 100).sum(),
    ])
    srs = np.argsort(suit_rank)
    # print(suit_size, suit_rank, srs)

    # print(hand)

    # print((hand // 100 == srs[0]+1) * (hand % 100 <= (14 - suit_size[srs[0]])) * 40)
    # print((hand // 100 == srs[1]+1) * (hand % 100 <= (14 - suit_size[srs[1]])) * 30)
    # print((hand // 100 == srs[2]+1) * (hand % 100 <= (14 - suit_size[srs[2]])) * 20)
    # print((hand // 100 == srs[3]+1) * (hand % 100 <= (14 - suit_size[srs[3]])) * 10)

    prio = (hand // 100 == srs[0]+1) * (hand % 100 <= (14 - suit_size[srs[0]])) * 40 \
        + (hand // 100 == srs[1]+1) * (hand % 100 <= (14 - suit_size[srs[1]])) * 30 \
        + (hand // 100 == srs[2]+1) * (hand % 100 <= (14 - suit_size[srs[2]])) * 20 \
        + (hand // 100 == srs[3]+1) * (hand % 100 <= (14 - suit_size[srs[3]])) * 10

    #print(prio)
    # reverse priority by deletion priority + card rank
    pp = 100-(prio + (14-(hand % 100)))
    # print(pp)
    ps = np.argsort(pp)
    # print(ps)
    # print(hand[ps][:2])

    return np.setdiff1d(hand, hand[ps][:2])
